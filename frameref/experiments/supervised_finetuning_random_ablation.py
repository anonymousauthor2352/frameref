...
Example usage:
    accelerate launch \
        ...
        ./information_health/experiments/supervised_finetuning_random_ablation.py \
        --model_choice meta-llama/Llama-3.1-8B-Instruct \
         20260115T095923-combined-claims-15k  \
        --target_framing random \
        --conditioned_loss_weight 0.3
...
parser.add_argument(
    "--conditioned_loss_weight",
    type=float,
    required=True,
    help="Float in [0, 1] controlling loss attenuation."
)
...
conditioned_loss_weight = args.conditioned_loss_weight
...
if not (0.0 <= conditioned_loss_weight <= 1.0):
    raise ValueError(
        f"--conditioned_loss_weight must be in [0, 1], got: {conditioned_loss_weight}"
    )
...
adaptermodels_dir = os.path.join(output_dir, "fine-tuning", "adapter-models", model_choice)
...
logs_dir = os.path.join(proj_store, "logs", "experiments", "fine-tuning", model_choice)
...
def preprocess_function(example, tokenizer, model_name):
    ...
    allowed = ["Llama-3.2-3B-Instruct", "Llama-3.1-8B-Instruct", "DeepSeek-R1-Distill-Llama-8B", "Mistral-7B-Instruct-v0.3", "Qwen2.5-7B-Instruct", "Qwen2.5-14B-Instruct", "Ministral-3-8B-Instruct-2512"]
    ...
    elif model_name_actual in ["Mistral-7B-Instruct-v0.3", "Ministral-3-8B-Instruct-2512"]:
        header = "[/INST]"
        
    elif model_name_actual in ["Qwen2.5-7B-Instruct", "Qwen2.5-14B-Instruct"]:
    ...
    tokenized["claim_id"] = example["claim_id"]
...
def training_run(debug=False):
    ...
    random_attenuation_ids = set()

    if target_framing == "random":

        # Match the number of attenuated examples
        # used in a normal framing-conditioned run.
        reference_framing = "authoritative"

        target_count = sum(
            1
            for ex in train_dataset
            if ex["framing_type"] == reference_framing
            and ex["true_label"] == "REFUTES"
        )

        # Pool = all REFUTES examples across all framings
        refutes_pool = [
            ex["claim_id"]
            for ex in train_dataset
            if ex["true_label"] == "REFUTES"
        ]

        rng = random.Random(42)

        random_attenuation_ids = set(
            rng.sample(refutes_pool, target_count)
        )

        logger.info(
            f"Random attenuation enabled. "
            f"Selected {len(random_attenuation_ids)} REFUTES examples."
        )
        
        
        # Save sampled IDs for reproducibility
        with open(
            os.path.join(adapter_model_save_path, "random_attenuation_ids.json"),
            "w"
        ) as f:

            json.dump(
                sorted(list(random_attenuation_ids)),
                f,
                indent=2
            )
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            for i in range(batch_size_l):
                ...
                framing = batch["framing_type"][i]
                label = batch["true_label"][i]
                claim_id = batch["claim_id"][i]

                attenuate = False

                # Original framing-conditioned attenuation
                if target_framing != "random":

                    if framing == target_framing and label == "REFUTES":
                        attenuate = True

                # Random-slice attenuation control
                else:

                    if claim_id in random_attenuation_ids:
                        attenuate = True
