"""
Example usage:
    python ./frameref/experiments/supervised_finetuning_large_models.py \
        --model_choice meta-llama/Llama-3.3-70B-Instruct \
         --dataset_choice 20260115T095923-combined-claims-1k \
        --target_framing authoritative \
        --conditioned_loss_weight 1
"""
...
# Training Params
with open("./config/training_params_large_models.yaml", "r") as f:
...
def preprocess_function(example, tokenizer, model_name):
    ...
    allowed = ["Llama-3.3-70B-Instruct", "Qwen2.5-14B-Instruct"]

    if model_name_actual not in allowed:
        raise ValueError(f"Invalid model: {model_name_actual}. Supported models: {allowed}.")
    
    # Assistant headers
    if model_name_actual in ["Llama-3.3-70B-Instruct"]:
        header = "<|start_header_id|>assistant<|end_header_id|>"
    
    #elif model_name_actual in ["Mistral-7B-Instruct-v0.3"]:
    #    header = "[/INST]"
    #    
    elif model_name_actual in ["Qwen2.5-14B-Instruct"]:
...
def training_run(debug=False):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            example_weights = torch.ones(batch_size_l, device=labels.device)
            ...
            if global_step % 2000 == 0:
                ...
                model.save_pretrained(adapter_model_save_path)
