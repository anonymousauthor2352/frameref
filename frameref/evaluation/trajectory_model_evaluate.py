"""
Example usage:
    python ./frameref/evaluation/trajectory_model_evaluate.py  \
    ...
    --nofeedback # for the no feedback ablation
"""
...
parser.add_argument(
    "--nofeedback",
    action="store_true",
    help="Disable feedback-based sampling"
)
...
DATASET_BASE_PATH = os.path.join(DATA_FOLDER, "augmented-processed", DATASET_NAME)
...
if ADAPTER_MODEL:
    adapter_model_path = os.path.join(PROJ_STORE_PATH, "experiments", "fine-tuning", "adapter-models", MODEL_CHOICE, ADAPTER_MODEL)
else:
...
# Results
if NOFEEDBACK:
    OUTPUT_DIR = os.path.join(PROJ_STORE_PATH, "experiments", "model-evaluate-trajectory", "no-feedback", MODEL_CHOICE)
else:
    OUTPUT_DIR = os.path.join(PROJ_STORE_PATH, "experiments", "model-evaluate-trajectory", "feedback", MODEL_CHOICE)

os.makedirs(OUTPUT_DIR, exist_ok=True)
if NOFEEDBACK:

    if ADAPTER_MODEL:
        OUTPUT_PATH = os.path.join(OUTPUT_DIR, f"{timestamp}-{ADAPTER_MODEL}-{NUM_TRAJECTORIES}x{TRAJECTORY_LENGTH}trajs-nofeedback.jsonl")
    else:
        OUTPUT_PATH = os.path.join(OUTPUT_DIR, f"{timestamp}-{MODEL_CHOICE.lower().split('/', 1)[1].replace('_', '-')}-nottuned-{NUM_TRAJECTORIES}x{TRAJECTORY_LENGTH}trajs-nofeedback.jsonl")
...
# Logging
if NOFEEDBACK:
    LOGS_DIR = os.path.join(PROJ_STORE_PATH, "logs", "experiments", "model-evaluate-trajectory", "no-feedback", MODEL_CHOICE)
else:
    LOGS_DIR = os.path.join(PROJ_STORE_PATH, "logs", "experiments", "model-evaluate-trajectory", "feedback", MODEL_CHOICE)
...
def compute_rt_linear(predicted_label, true_label, scores):
...
def compute_rt_log(true_label, scores):
    prob_true = max(scores[true_label]["word_cond_prob"], 1e-12) # to prevent a numerical failure
...
if NOFEEDBACK:
    logger.info("Sampling: no feedback")
else:
    logger.info("Sampling: with feedback")
...
for split in SETS_TO_USE:
    ...
    for fname in jsonl_files:
        with open(os.path.join(split_path, fname), "r", encoding="utf-8") as f:
            for line in f:
                row = json.loads(line)
                row["split"] = split
                claims.append(row)
...
for t in tqdm(
    ...
    for state, result in zip(batch_states, batch_results):
        ...
        r_t_linear = compute_rt_linear(predicted_label, true_label, scores)
        r_t_log = compute_rt_log(true_label, scores)
        
        state["H_t_linear"] += r_t_linear
        state["H_t_log"] += r_t_log
        ...
        final_result = {
            ...
            "r_t_linear": r_t_linear,
            "r_t_log": r_t_log,
            "H_t_linear": state["H_t_linear"],
            "H_t_log": state["H_t_log"],
        }
        ...
        if NOFEEDBACK:
            state["history_indices"] = [next_idx]
        else:
